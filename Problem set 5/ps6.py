...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        '''
        result = {}
        alhpabet = [letter for letter in string.ascii_lowercase]
        l = len(alhpabet)
        for i in range(l):
            j = i + shift
            if j >= l:
                j -= l
            result[alhpabet[i]] = alhpabet[j]
            result[alhpabet[i].upper()] = alhpabet[j].upper()
        return result
    ...
    def apply_shift(self, shift):
        ...
        for letter in self.message_text:
            result += dict_map.get(letter, letter)
        return result
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        for s in range(l):
            message_text_decrypted = self.apply_shift(l - s)
            count = 0
            ...
            if count > max_num:
                ...
                if s == 0:
                    best_shift = 0
                best_text_decrypted = message_text_decrypted
            if count == len(message_text_decrypted.split()):
                break
